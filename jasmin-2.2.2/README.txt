--------------------------------------------------------------------------
Jasmin README file			   Nov 20, 2006, Sigmund Cherem
--------------------------------------------------------------------------

F-Rex extensions to jasmin. F-Rex uses jasmin as a back-end (via soot).
All modifications are to support 11 new instructions in the java bytecodes.
Most bytecodes from 202 to 255 are unused, we assigned the following
bytecodes:

// for jreg
    230 create_ru : create an unbounded region
    231 create_rb : create a bounded region
    232 remove_r  : remove a region
    233 new_in_r  : allocate in a region
    234 anewarray_in_r : allocate an ref array in a region
    235 newarray_in_r  : allocate a basic array in a region
    236 multianewarray_in_r : allocate a multidimentional array in a region
    237 push_r : push a region onto the stack
    238 begin_fork_r : begin creating a new thread: pushing copies regions
    239 end_fork_r : ends creating the new thread.

// for jfree
    240 free_o : deallocates an individual object instance

Below is the original readme file distributed with soot.
