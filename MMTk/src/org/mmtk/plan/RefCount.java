 ...
 * collector.
 *
 * $Id: RefCount.java,v 1.10 2005/02/04 10:03:07 steveb-oss Exp $
 *
...
public class RefCount extends RefCountBase implements Uninterruptible {
  ...
  public final Address alloc(int bytes, int align, int offset, int allocator)
    ...
    Address res;
    switch (allocator) {
        case       ALLOC_RC: res = rc.alloc(bytes, align, offset, false);
  ...
  private final void writeBarrierInternal(ObjectReference src, Address slot,
    ...
    if (WITH_COALESCING_RC) {
      ...
      ObjectReference old = Barriers.performWriteInBarrierAtomic(src, slot, tgt, metaDataA, metaDataB, mode);
